...
This dissertation addresses that gap through a controlled, within-subjects experiment. Four progressively refined pipeline configurations are applied to a set of real open-source GitHub Actions projects: a baseline (C1), dependency caching (C2), workflow consolidation (C3), and caching plus consolidation combined (C4). Path-based trigger filtering was part of the combined configuration's original design but, because every run in the protocol is triggered manually via `workflow_dispatch` to allow an exact, controlled run count, no workflow file ever exercises a path-filtered trigger; its carbon effect is therefore not measured by this study (Section 3.3). Energy is measured directly inside CI using the Eco-CI energy estimation tool, and carbon is computed using the Software Carbon Intensity specification (ISO/IEC 21031:2024). The study answers three research questions: the carbon reduction attributable to each strategy (RQ1), the consistency of those savings across project languages and sizes (RQ2), and the carbon saved per unit of implementation effort (RQ3).
...
Second, the Eco-CI Energy Estimation tool (Green Coding Solutions, 2023) makes energy measurement inside cloud CI environments practically achievable. GitHub-hosted runners do not expose hardware-level energy counters; Eco-CI addresses this by using a machine learning model trained on the SPECpower database to estimate energy consumption from CPU utilisation data, producing per-stage energy measurements in joules without requiring physical instrumentation. A 2026 IEEE study applying this class of tool to 204 open-source Java projects reports that enabling dependency caching reduced CI energy consumption by 30% on average for Maven projects and by over 90% in some Gradle cases (IEEE, 2026); this citation's bibliographic record could not be independently re-verified at the time of writing (Section 2.2.3), so the specific figures are treated as indicative rather than settled throughout this dissertation.
...
Saavedra, Mendes and Ferreira (2025), in "Environmental Impact of CI/CD Pipelines," apply the Cloud Carbon Footprint (CCF) framework to quantify the carbon and water footprints of the entire GitHub Actions ecosystem in 2024, estimating an annual footprint of 456.9 MTCO₂e and identifying geographic runner placement as the single largest lever available to reduce it.
...
This paper establishes the significance of the problem at scale, but stops short of the project-level question this dissertation takes up directly: how should an individual maintainer actually configure their pipeline?

### 2.2.2 Large-Scale Energy Analysis of GitHub Workflows

Alves et al. (2024), in "Software Frugality in an Accelerating World: the Case of Continuous Integration," conduct the first large-scale analysis of the energy consumption of GitHub Actions workflows by executing workflows locally on a controlled server to measure their energy consumption directly. The central problem the paper identifies is visibility: aggregate CI energy consumption turns out to be highly skewed across projects, yet developers have no tooling that surfaces this cost at the point where they actually make configuration decisions.
...
Together, these three studies establish that the CI/CD carbon problem is real, large, and skewed towards a minority of intensive projects, and that at least one intervention (caching) already has strong empirical backing in a single-language context. None of the three, however, tests more than one strategy, and none applies a standardised carbon metric consistently across configurations. Closing that specific gap is exactly what this dissertation's four-configuration design is built to do.

## 2.3 Energy Profiling of Individual CI Stages

### 2.3.1 Build Automation Tool Energy Profiles

De Medeiros, Lefeuvre, Combemale and Perez (2025), in "Evaluating the Energy Profile of Tasks Managed by Build Automation Tools in Continuous Integration Workflows," investigate the energy consumption of tasks managed by Apache Maven and Gradle within GitHub Actions CI workflows, using the SmartWatts hardware performance counter tool at 2 Hz sampling. Testing turns out to be the dominant CI energy stage (36 to 47% of total), and, notably, project size does not reliably predict per-task energy consumption.
...
Niang (2024), in "CI/CD Pipelines: Good Software Development Practice, But Green?," examines CI/CD pipeline energy consumption by varying three parameters (CI platform, programming language, and build tool) using a single small-scale application of 455 lines of code: GitHub Actions versus GitLab CI/CD, Java versus Python. The headline result is that GitHub Actions is measurably less energy-intensive than GitLab CI/CD for an identical workload, and that Eco-CI is a workable instrument for capturing that difference.
...
The study uses a within-subjects design: each project is measured under all four configurations (C1 to C4) against the same frozen subject code, enabling comparisons that eliminate between-project variability as a confound. The pairing between individual runs across configurations is by chronological trigger order rather than by a shared identifier, a weaker matching criterion than a true repeated-measures design; Section 5.2 reports a robustness check against this assumption.
...
A note on the Retrofit repository specifically: the project is checked out from `lysine-dev/retrofit`, not the `square/retrofit` location under which it was originally created and is still widely referenced. Ownership of the repository transferred to the `lysine-dev` organisation; the repository itself, its full history, and its 43,900+ stars are unchanged and confirmed current via the GitHub API at the time of writing. The reference cited is accurate for the repository as it exists today.
...
The primary comparison uses Wilcoxon signed-rank tests to compare total energy per CI run between C1 (baseline) and each of C2, C3, and C4, pairing runs by chronological trigger order within each project, since no other shared identifier links a C1 run to a "corresponding" C2, C3, or C4 run: they are independently triggered executions of different workflow files, not a shared subject measured twice. This ordinal pairing is a deliberate design choice, made because it uses the within-subjects structure of triggering all four configurations against the same fixed, frozen subject code under matched conditions (same day, same 5-minute inter-run interval, same runner type), but it is a weaker form of pairing than a true repeated-measures design, since "run 7 of C1" and "run 7 of C2" are matched by position in a triggering sequence rather than by any shared identity. As a robustness check against this assumption, every comparison in Table 5.3 was re-run as an unpaired Mann-Whitney U test, which makes no pairing assumption at all: every one of the fifteen comparisons agrees with its Wilcoxon counterpart on statistical significance at the Bonferroni-corrected threshold, with no flips in either direction. This does not make the ordinal pairing a true repeated-measures design, but it shows that none of this dissertation's conclusions depends on that pairing assumption being exactly correct. The remaining threats from shared-infrastructure noise are addressed separately in Section 6.6. Because three comparisons are made per project, a Bonferroni correction adjusts the significance threshold to α = 0.05 / 3 = 0.0167. Effect size is reported with Cliff's delta (δ), interpreted following Romano et al. (2006): |δ| below 0.147 negligible, below 0.330 small, below 0.474 medium, and 0.474 or above large.
...
The answer to RQ2 is clear, if not the one this study's cross-language design was expecting to find: **the savings are not consistent across projects.** The combined configuration's effect ranges from a large, significant −33.70% reduction in Retrofit down to a negligible, non-significant +0.47% in resty, a difference of thirty-four percentage points between the best and worst case. Table 5.4 summarises.
...
Dependency caching (C2) produces a statistically significant, large-effect energy reduction in three of the five projects studied: HTTPie (−4.56%, δ = 0.709), Retrofit (−33.85%, δ = 0.998), and Gson (−18.53%, δ = 0.964). It produces no significant effect in got (−1.50%) or resty (+2.91%, wrong direction, though not significant). HTTPie's per-stage breakdown (Table 5.2) shows exactly the mechanism expected: dependency installation falls from 463.36 J (C1) to 388.35 J (C4), a 16.2% reduction, while test execution barely moves (1,008.45 J to 981.61 J, 2.7%). This is mechanistically consistent with de Medeiros et al. (2025), who identify testing as CI's dominant energy stage, and directionally consistent with the IEEE (2026) Java study's report that caching is the highest-impact single intervention it tested, though that citation's own unresolved provenance (Section 2.2.3) means it corroborates the direction of this dissertation's finding rather than its magnitude. Retrofit's exceptionally large caching effect (a 33.85% reduction, the largest single result in this study) is consistent with the same mechanism operating on a project with a substantially heavier Gradle dependency graph than HTTPie's pip requirements.
...
**Construct validity.** Eco-CI estimates CPU energy only; network and disk activity during dependency installation are not captured, so the measured caching reductions are a lower bound on the true energy savings. Path filtering's carbon effect is entirely unmeasured, for the reason given in Section 3.3 (every run is `workflow_dispatch`-triggered, never a path-filtered `push`); Table 3.3's description of C4 as including path filtering should be read as a design intention that measurement never exercised, not a result.
