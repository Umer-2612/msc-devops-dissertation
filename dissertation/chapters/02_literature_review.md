...
Critically for this dissertation, the study finds that enabling dependency caching reduced CI energy consumption by 30% on average in Maven projects, and by over 90% in some Gradle cases. This constitutes the strongest empirical validation currently available for dependency caching as a high-impact CI refinement strategy, and directly supports the design rationale for including caching as Configuration C2 in this dissertation's experiment. The study is restricted to Java projects using Maven/Gradle; this dissertation extends the investigation to Python projects and adds the orthogonal strategies of workflow consolidation and trigger filtering.
...
This dissertation directly addresses that identified future work. Where de Medeiros et al. profile energy statically, this dissertation applies caching and measures before-and-after differences. Where they restrict to Java/Maven/Gradle, this dissertation tests Python projects, enabling a cross-language comparison.

### 2.3.2 Platform and Language Energy Comparison

Niang (2024) examines CI/CD pipeline energy consumption by varying three parameters — CI platform (GitHub Actions vs GitLab CI/CD), programming language (Java vs Python), and build tool — using a single small-scale application of 455 lines of code. Energy is estimated using the Eco-CI tool. Key findings: GitHub Actions averaged 13.36 joules per execution compared to GitLab's 21.81 joules; Java averaged 11.36 joules versus Python's 14.62 joules. The study acknowledges its primary limitation explicitly: a single tiny project with no statistical generalisation.
